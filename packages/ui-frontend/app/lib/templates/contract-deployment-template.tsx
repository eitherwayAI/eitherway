...
// Use the public EitherWay API endpoint
// This works from WebContainer AND deployed apps
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://dev.eitherway.ai';
...
export class ContractService {
  ...
  /**
   * Compile a smart contract using the public EitherWay API
  ...
  async compileContract(request: CompileContractRequest): Promise<CompileContractResponse> {
    try {
      console.log(\`[ContractService] Compiling contract via: \${this.apiBaseUrl}/api/contracts/compile\`);

      const response = await fetch(\`\${this.apiBaseUrl}/api/contracts/compile\`, {
        method: 'POST',
        headers: {
      ...
      return {
        success: false,
        error: \`Failed to connect to backend API: \${error.message}. Check if dev.eitherway.ai is accessible.\`
      };
    }
  }
...
export const wagmiAdapter = new WagmiAdapter({
  networks: [sepolia, baseSepolia, arbitrumSepolia],
  projectId
});
...
export default defineConfig({
  ...
  // CRITICAL: Permissive CORS for WebContainer and Web3 wallet compatibility
  // NOTE: Do NOT add COEP headers here - they break npm install in WebContainer
  server: {
...
# Uses the public API endpoint - works from anywhere (WebContainer & deployed apps)
VITE_API_BASE_URL=https://dev.eitherway.ai
...
// This .env file is OPTIONAL - the app has sensible defaults:
// - VITE_API_BASE_URL: Defaults to https://dev.eitherway.ai (public EitherWay API)
// - VITE_WALLETCONNECT_PROJECT_ID: Uses demo project ID if not specified
...
## Setup:
1. (Optional) Create .env file with VITE_API_BASE_URL=https://dev.eitherway.ai
2. Get testnet ETH in your wallet from faucets:
