...
ENVIRONMENT VARIABLES - AUTOMATIC CONFIGURATION:
  ...
  1. Create /.env file with this content (OPTIONAL - app has sensible defaults):
     \`\`\`
     # EitherWay Backend API (for contract compilation)
     # Uses public API endpoint - works from WebContainer and deployed apps
  ...
  - Import networks from '@reown/appkit/networks' NOT 'viem/chains'
  - Use 'networks' property NOT 'chains' (API changed in Reown AppKit)
  - Do NOT include 'transports' object (no longer needed)
  - Include 'networks' in both WagmiAdapter AND createAppKit
...
COMMON MISTAKES TO AVOID:
  ...
  ❌ Adding COEP/COOP headers to vite.config.js (breaks npm install)
  ❌ Using 'chains' instead of 'networks' in WagmiAdapter (old API)
  ❌ Importing from 'viem/chains' instead of '@reown/appkit/networks'
  ❌ Including 'transports' object in WagmiAdapter (no longer needed)
  ❌ Using localhost URLs instead of public dev.eitherway.ai API
  ...
  ✅ Use 'networks' property (not 'chains') in WagmiAdapter and createAppKit
  ✅ Create .env with VITE_API_BASE_URL and VITE_WALLETCONNECT_PROJECT_ID
  ✅ Add CORS headers to vite.config.js (cors: true, host: true)
